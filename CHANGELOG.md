...
- Set `SHOW_COLLAPSED` and `UPDATE_ON_FETCH` to `django-debug-toolbar` config, to prevent the toolbar from showing on all navigation and to make sure it's updated on HTMX fetch requests.
